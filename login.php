...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // check empty fields (extra validation)
    if (empty($email) || empty($pass)) {
        ...
        //  CHECK DISABLED FIRST (important logic fix)
        else if ($user['disabled'] == 1) {
            $message = "Your account has been disabled by admin.";
        }
        //  PASSWORD CHECK (better structure)
        ...
        // LOGIN SUCCESS
            ...
            //  DIFFERENT REDIRECTS (better logic)
            if ($user['role'] === 'admin') {
...
<body>
    ...
    <!--Code for logging in if user is not found then tell them to create an accoutn-->

    <!--Code for loggin in if user is admin then show things that only admin can see/access
    which is the  admin.php in the header-->
